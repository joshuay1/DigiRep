...
\title{Evidence Asymmetry in Legislative Digital Twins: Augmenting Swiss Parliamentary Vote Prediction with Public Interest Records}
...
\begin{abstract}
This paper investigates whether large language model (LLM)-based agents can serve as behavioral digital twins of Swiss parliamentarians by predicting roll-call votes across policy domains, parties, and data-scarce settings. We focus on a central failure mode: evidence asymmetry. Public textual evidence is not equally available or equally informative across parties and topics, which can make some parliamentary actors harder to simulate than others. We first compare two approaches: a black-box predictor obtained by fine-tuning a GPT-OSS 20B model on historical roll-call data, and an interpretable agent pipeline that constructs a knowledge graph for each party-topic pair from speeches, roll calls, manifestos, and press releases. Preliminary results suggest that both approaches perform unevenly across parties and policy domains. We therefore propose an affiliation-augmented methodology that incorporates public records of lawmakers' declared interests, firm links, and political financing as structured evidence. The central hypothesis is that such records provide complementary predictive signal when textual party-topic evidence is sparse. We discuss how to evaluate this hypothesis without making causal claims about influence, and identify methodological challenges including vote polarity inversions, sparse party-topic cells, evidence provenance, and political pluralism.
\end{abstract}
...
\caption{Summary of the methodological tradeoff between predictive performance, interpretability, and evidence augmentation.}
...
Political digital twins present clear risks. They may be mistaken for authentic representatives, used to manipulate political messaging, or evaluated only by predictive accuracy while ignoring explanation quality and democratic legitimacy. Affiliation-augmented systems add another risk: they may invite overinterpretation of firm links as proof of influence. Any deployment should therefore mark outputs as simulations, preserve provenance for all evidence, report uncertainty, and avoid presenting a generated prediction as a substitute for accountable political judgment.
