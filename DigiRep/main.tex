...
THIS TEXT IS AI-GENERATED AND WILL BE ENTIRELY  REMOVED IN THE NEXT WEEKS
...
Political institutions generate rich traces of legislative behavior: roll-call votes, speeches, manifestos, press releases,
and public records about interests and political financing.

These traces raise a natural question for computational social science and AI: can LLM-based agents approximate the voting behavior of elected representatives well enough to
function as behavioral digital twins for analysis and simulation?

This question is technically and normatively demanding. The prediction task is not merely to classify text. A parliamentary vote can reflect ideology, party discipline, issue
...
We study roll-call prediction as a three-way classification task: Yes, No, or Abstention. The central empirical question is how LLM-based predictors behave as the amount and
kind of available evidence varies. We compare a fine-tuned
black-box model with an interpretable, retrieval-oriented
agent based on party-topic knowledge graphs. We then ask
whether public records of firm links, declared interests, and
political financing can reduce performance gaps in sparse or
underrepresented settings.

This framing is useful in three legislative settings. First,
policy questions are often new or only weakly represented
in historical roll calls. In such cases, exploring how parties have collectively decided on similar issues, using unstructured evidence produced by the parties themselves
and aligned with their stated values, can support decisionmaking. Accurate party-level prediction in data-scarce settings is therefore our main empirical focus. Second, individual and collective preferences are not fully captured by rollcall labels. Speeches, manifestos, and press releases contain richer information about issue framing, exceptions, and
intra-party priorities. Our knowledge graphs move in this direction by summarizing party- and lawmaker-level evidence
in a form that can be inspected rather than only predicted
from. Third, more ambitious democratic digital twins could
help parties integrate diverse or contrarian viewpoints by
generating lawmaker- or party-specific statements and then
summarizing areas of consensus and disagreement. Public
press releases and position papers could provide a natural
benchmark for evaluating whether such summaries resemble real political communication while preserving pluralistic
representation. This third scenario is beyond the present empirical scope, but it motivates our emphasis on provenance,
uncertainty, and pluralistic outputs.
...
\paragraph{Representation congruence and interest exposure.}
Political representation is usually evaluated not only by whether elite behavior is predictable, but by whether parties and representatives remain congruent with citizens and constituencies. Work on party--voter congruence shows that representation varies strongly across issues and is often higher on issues parties emphasize during campaigns \citep{costello2021congruenceVotersParties}. Measurement work further cautions that congruence estimates depend on which policy statements are sampled and how topically diverse those statements are \citep{lesschaeve2023measuringAgreement}. In the Swiss context, direct-democratic institutions and representative institutions can achieve policy congruence through distinct mechanisms \citep{leemann2013directDemocracyPolicyCongruence}. Finally, work on campaign contributions and climate roll-call votes shows how interest-group exposure can be studied as a measurable distortion in legislative behavior rather than merely asserted as a political concern \citep{gaoHuang2026carbonClimateCongress}. These literatures sharpen our framing: the goal is not only to predict roll calls, but to identify where political evidence is missing, where parties may underrepresent salient issues, and where public affiliation records may help audit potential asymmetries.
...
\paragraph{AI for explainable politics.}
The broader AI-and-democracy literature makes the democratic stakes of these design choices explicit. \citet{jungherr2023artificialIntelligenceDemocracy} argues that analysis should focus on concrete narrow-AI systems and their effects on self-rule, equality, elections, and competition between political systems rather than on speculative artificial general intelligence. In public policymaking, explainability and transparency are treated as operational requirements for trustworthy, regulation-aware AI systems \citep{papadakis2024explainableTransparentAI}. In legislative communication, AI-mediated correspondence can increase perceived responsiveness relative to generic replies, but only under conditions of quality, disclosure, and human oversight \citep{kreps2023aiCommunicationTools}. Recent work on AI and democratic representation similarly argues that AI shifts representatives' roles across input representation, where citizen preferences are aggregated, and output representation, where policies are implemented through algorithmic systems \citep{rymon2026peopleAlgorithm}. Our contribution fits this direction by treating explainability as a democratic audit problem: a political AI system should expose the evidence, uncertainty, and representational assumptions behind its predictions.
...
\textbf{Representation audit.} We will connect prediction errors and evidence gaps to party-topic salience, public opinion where available, and affiliation records. This would move the analysis from ``can we predict the vote?'' toward ``where is representation well evidenced, weakly evidenced, or potentially distorted?''
